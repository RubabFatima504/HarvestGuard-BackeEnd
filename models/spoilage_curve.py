...
def generate_spoilage_curve(
    ...
    # v3 fix: crop_profiles.json mein har crop ka "critical_temp_celsius"
    # hai (research-cited threshold jiske upar decay dangerous ho jata
    # hai) - lekin ye field ab tak kahin bhi use nahi ho rahi thi. Isi
    # wajah se early-days batches (jaise 3 din purana potato 38C pe)
    # hamesha LOW dikhte the sirf isliye ke abhi elapsed time kam hai,
    # chahe current temperature crop ke apne documented danger-threshold
    # se upar kyun na ho. Pehle ye gap Advisory LLM apni marzi se prose
    # mein "practical risk ko Medium tak raise kar raha hoon" likh ke
    # patch kar raha tha - jo top-level risk_level badge se contradict
    # karta tha (frontend pe LOW aur MEDIUM dono dikhte the).
    # Fix: agar current_temp crop ke apne critical_temp_celsius ko cross
    # kar chuki hai, risk_score ko ek floor do (min 50 -> MEDIUM range)
    # taake ye judgment deterministic ho, LLM ki prose mein chhupi na ho.
    critical_temp = crop_profile.get("critical_temp_celsius")
    temp_is_critical = critical_temp is not None and current_temp >= critical_temp
    ...
    return {
        ...
        "temp_is_critical": temp_is_critical,
    }