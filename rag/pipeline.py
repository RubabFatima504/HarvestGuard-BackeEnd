...
"""
RAG Pipeline — TF-IDF based retrieval (no local model, no external API).
Chosen deliberately for a small (9-doc) domain-specific knowledge base
running on a 512MB instance — semantic embeddings weren't buying us
enough over TF-IDF to justify the memory cost of any model (local or
API-based) that has to live in process memory or cost money per call.
"""
from pathlib import Path
from dotenv import load_dotenv
from langchain_community.document_loaders import DirectoryLoader, TextLoader
...
def build_vector_store():
    ...
    knowledge_base_path = Path(__file__).parent / "knowledge_base"
    loader = DirectoryLoader(
        str(knowledge_base_path), glob="**/*.md",
    ...
    chunk_texts = [c.page_content for c in _chunks]
    _vectorizer = TfidfVectorizer(stop_words="english")
    _tfidf_matrix = _vectorizer.fit_transform(chunk_texts)
    print(f"[RAG] TF-IDF index built — {len(_chunks)} chunks, no model download needed")
